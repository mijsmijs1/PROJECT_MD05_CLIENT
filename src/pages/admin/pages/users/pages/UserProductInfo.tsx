...
export default function UserProductInfo() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await api.product.getStatusCount(userId);
                if (res.status === 200) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            if (status) {
                try {
                    console.log(status);
                    const res = await api.product.getAllProduct(userId, status, (page != 0) ? Number(page) : 1);
                    if (res.status == 200) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const res = await api.user.getUserByIdByAdmin(userId);
                console.log(res);

                if (res.status == 200) {
                    dispatch(authenAction.setData(res.data.data));
        ...
        <div className='user_info_box'>
            <div className='user'>
                <div className='user_left'>
                    <img
                        src={authenStore.data?.avatar.includes("img/") ? `${import.meta.env.VITE_SV_HOST}/${authenStore.data?.avatar}` : authenStore.data?.avatar}></img>
